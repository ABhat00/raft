...
class Replica:
    ...
    def run(self, rids, silence):
        ...
        # the main process
        if silence: self.proc = subprocess.Popen(args, stdout=DEVNULL, stderr=DEVNULL, preexec_fn=os.setsid, shell=True)
        else: self.proc = subprocess.Popen(args, preexec_fn=os.setsid, shell=True)
